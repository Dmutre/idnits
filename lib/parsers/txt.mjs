...
const BRACKETED_RFC_REFERENCE_RE = /\[RFC(\d+)\]/gi
...
const TABLE_OF_CONTENTS = /Table\s+of\s+Contents/i
const NUMBER_WITH_DOTS_RE = /^(\d+(\.\d+)*\.)/
const CONTENT_LINE_RE = /^(\d+(\.\d+)*\.)\s{2,}.*\.\s{2,}\d+$/
const CONTENTS_CONTINUATION_RE = /^\s{3,}\S/
const NUMBER_BEFORE_SPACE_RE = /^\s+\d/
...
const licenseDeclarations = {
  ...
  revised_bsd_license6_i: /This document is subject to BCP 78 and the IETF Trust.s Legal Provisions Relating to IETF Documents \(https?\s*:\s*\/\/\s*trustee\s*\.\s*ietf\s*\.\s*org\s*\/\s*license\s*-\s*info\) in effect on the date of publication of this document\. Please review these documents carefully, as they describe your rights and restrictions with respect to this document\. Code Components extracted from this document must include (Simplified|Revised) BSD License text as described in Section 4\.e of the Trust Legal Provisions and are provided without warranty as described in the (Simplified|Revised) BSD License\./g,
  copyright_section6_b_i: /(Copyright\s+\(c\)\s+(\d{4})\s+IETF Trust and the persons identified as the\s+document authors\.\s*All rights reserved\.|Copyright\s+\(C\)\s+The Internet Society\s+\((\d{4})\)\.\s*All Rights Reserved\.)/gi,
  license6_b_ii: /This document is subject to BCP 78 and the IETF Trust’s Legal Provisions Relating to IETF Documents \(https?\s*:\s*\/\/\s*trustee\s*\.\s*ietf\s*\.\s*org\s*\/\s*license\s*-\s*info\) in effect on the date of publication of this document\. Please review these documents carefully, as they describe your rights and restrictions with respect to this document\./g,
  licence6_c_i: /This document may not be modified, and derivative works of it may not be created, except to\s+format it for publication as an RFC or to translate it into languages other than English\./,
  licence6_c_ii: /This document may not be modified, and derivative works of it may not be created, and it may not\s+be published except as an Internet-Draft\./,
  ietf_contributions_license6_c_iii: /This\s+document\s+may\s+contain\s+material\s+from\s+IETF\s+Documents\s+or\s+IETF\s+Contributions\s+published\s+or\s+made\s+publicly\s+available\s+before\s+November\s+10,\s+2008\.\s+The\s+person\(s\)\s+controlling\s+the\s+copyright\s+in\s+some\s+of\s+this\s+material\s+may\s+not\s+have\s+granted\s+the\s+IETF\s+Trust\s+the\s+right\s+to\s+allow\s+modifications\s+of\s+such\s+material\s+outside\s+the\s+IETF\s+Standards\s+Process\.\s+Without\s+obtaining\s+an\s+adequate\s+license\s+from\s+the\s+person\(s\)\s+controlling\s+the\s+copyright\s+in\s+such\s+materials,\s+this\s+document\s+may\s+not\s+be\s+modified\s+outside\s+the\s+IETF\s+Standards\s+Process,\s+and\s+derivative\s+works\s+of\s+it\s+may\s+not\s+be\s+created\s+outside\s+the\s+IETF\s+Standards\s+Process,\s+except\s+to\s+format\s+it\s+for\s+publication\s+as\s+an\s+RFC\s+or\s+to\s+translate\s+it\s+into\s+languages\s+other\s+than\s+English\./g,
  ...
  draft_paragraph_pointing_to_the_list_of_current_ids: /The list of current Internet-\s*Drafts is at https:\/\/datatracker\.ietf\.org\/drafts\/current\/|The list of current Internet-Drafts can be accessed at http:\/\/www\.ietf\.org\/ietf\/1id-abstracts\.txt\./gi,
  ...
  acceptable_paragraph_noting_that_draft: /Internet-Drafts are working documents of the Internet Engineering Task Force \(IETF\)(?:, its areas, and its working groups)?\.\s*Note that other groups may also distribute working documents as Internet-Drafts(?:\.\s*The list of current Internet-Drafts is at https?:\/\/datatracker\.ietf\.org\/drafts\/current\/?\s*\.)?/g
}
...
const MAX_LINES_WITHOUT_TITLE = 18
...
export async function parse (rawText, filename) {
  ...
  try {
    ...
    // Search for rfc 2119 keyword declaration boilerplate
    ...
    data.contains.licencse6_c_iii = licenseDeclarations.ietf_contributions_license6_c_iii.test(normalizedText)

    // Searching for submission compliance line
    data.contains.submissionCompliance = licenseDeclarations.trust_28_dec_2009_section_6_a.test(normalizedText)
    ...
    const match = [...normalizedText.matchAll(licenseDeclarations.copyright_section6_b_i)]
    ...
    const match6bi = [...normalizedText.matchAll(licenseDeclarations.revised_bsd_license6_i)]
    const match6bii = [...normalizedText.matchAll(licenseDeclarations.license6_b_ii)]
    if (match6bii.length > 0) {
      data.extractedElements.licence6_b_ii = match6bii.map(m => m[0])
    }
    if (match6bi.length > 0) {
      data.contains.revisedBsdLicense6_i = true
      data.extractedElements.licence6_b_i = match6bi.map(m => m[0])
    }

    // Serach for 6.c licenses declaration
    data.contains.licence6_c_i = licenseDeclarations.licence6_c_i.test(normalizedText)
    data.contains.licence6_c_ii = licenseDeclarations.licence6_c_ii.test(normalizedText)
    ...
    for (const line of rawText.split('\n')) {
      ...
      if (FIRST_LINE_RE.test(trimmedLine) && lineIdx === 1) {
        data.extractedElements.isPKorBM = true
      }
      ...
      if ((data.title && lineIdx <= markers.title + 3) || (!data.title && lineIdx < MAX_LINES_WITHOUT_TITLE)) {
      ...
      // Check section title unexpected indentation
      // --------------------------------------------------------------
      if (PAGE_BREAK_RE.test(line)) {
        pageBreakDetected = true
        continue
      }

      if (pageBreakDetected && trimmedLine !== '') {
        pageBreakDetected = false
        continue
      }
      ...
      const isContentsLine = trimmedLine.match(CONTENT_LINE_RE)
      const isContentsContinuation = isTableOfContents && trimmedLine.match(CONTENTS_CONTINUATION_RE)

      if (trimmedLine.match(NUMBER_WITH_DOTS_RE) && NUMBER_BEFORE_SPACE_RE.test(line) && !isTableOfContents && !isContentsLine) {
        data.possibleIssues.unexpectedIndentation.push({
          line: lineIdx,
          pos: line
        })
      }

      if (isTableOfContents && !isContentsLine && !isContentsContinuation) {
        isTableOfContents = false
      }

      if (isTableOfContents || isContentsLine || isContentsContinuation) continue

      const leadingSpaces = line.match(LEADING_SPACES_RE)[0].length

      if (lastWasHeading && leadingSpaces >= 6) {
        lastWasContinuation = true
        continue
      }

      if (lastWasHeading && !lastWasContinuation) {
        lastWasHeading = false

        if (leadingSpaces !== 3 && !trimmedLine.startsWith('Internet-Draft')) {
          data.possibleIssues.unexpectedIndentation.push({
            line: lineIdx,
            pos: line
          })
