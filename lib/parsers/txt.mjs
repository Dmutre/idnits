...
const licenseDeclarations = {
  ...
  revised_bsd_license6_i: /This document is subject to BCP 78 and the IETF Trust.s Legal Provisions Relating to IETF Documents \(https?\s*:\s*\/\/\s*trustee\s*\.\s*ietf\s*\.\s*org\s*\/\s*license\s*-\s*info\) in effect on the date of publication of this document\. Please review these documents carefully, as they describe your rights and restrictions with respect to this document\. Code Components extracted from this document must include (Simplified|Revised) BSD License text as described in Section 4\.e of the Trust Legal Provisions and are provided without warranty as described in the (Simplified|Revised) BSD License\./,
  copyright_section6_b_i: /(Copyright\s+\(c\)\s+(\d{4})\s+IETF Trust and the persons identified as the\s+document authors\.\s*All rights reserved\.|Copyright\s+\(C\)\s+The Internet Society\s+\((\d{4})\)\.\s*All Rights Reserved\.)/gi,
