...
const STRICT_RFC_REFERENCE_RE = /\[RFC(\d+)\]/gi
...
export async function parse (rawText, filename) {
  ...
  try {
    ...
    for (const line of rawText.split('\n')) {
      ...
      while ((rfcMatch = (currentSection === 'references' ? STRICT_RFC_REFERENCE_RE : RFC_REFERENCE_RE).exec(trimmedLine)) !== null) {
      ...
      if (data.extractedElements.referenceSectionRfc.length > 0) {
        data.references.type = 'normative'
      } else {
        const numericDraftReference = data.extractedElements.referenceSectionDraftReferences.some((ref) => {
          return NUMERIC_REFERENCE_RE.test(ref.value || ref)
        })

        if (numericDraftReference) data.references.type = 'informative'
      }
