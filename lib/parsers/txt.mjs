...
export async function parse (rawText, filename) {
  ...
  try {
    ...
    for (const line of rawText.split('\n')) {
      ...
      const copyrightLineMatch = [...line.matchAll(licenseDeclarations.copyright_section6)]
      const copyrightLicenseMatch = [...line.matchAll(licenseDeclarations.copyright_license)]
      ...
      if (copyrightLineMatch.length) {
        ...
        data.extractedElements.copyrightDates = copyrightLineMatch.map(m => m[1] || m[2])
        const copyrightYears = match.map(m => Number(m[1]) || Number(m[2]))
