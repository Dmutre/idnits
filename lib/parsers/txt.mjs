...
export async function parse (rawText, filename) {
  ...
  try {
    ...
    const copyrightLineMatch = [...normalizedText.matchAll(licenseDeclarations.copyright_section6_b_i)]
    // Searching for copyright license
    const copyrightLicenseMatch = [...normalizedText.matchAll(licenseDeclarations.revised_bsd_license6_i)]

    if (copyrightLicenseMatch.length) {
      data.possibleIssues.copyrightLicenses = copyrightLicenseMatch.map(m => m[0])
    }

    if (copyrightLineMatch.length) {
      data.possibleIssues.copyrightLines = copyrightLineMatch.map(m => m[0])
    }
