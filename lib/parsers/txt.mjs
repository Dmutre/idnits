...
const licenseDeclarations = {
  ...
  copyright_license6_b_i: /This document is subject to BCP 78 and the IETF Trust’s Legal Provisions Relating to IETF Documents \(http:\/\/trustee\.ietf\.org\/license-info\) in effect on the date of publication of this document\. Please review these documents carefully, as they describe your rights and restrictions with respect to this document\. Code Components extracted from this document must include Simplified BSD License text as described in Section 4\.e of the Trust Legal Provisions and are provided without warranty as described in the Simplified BSD License\.|This document is subject to BCP 78 and the IETF Trust’s Legal Provisions Relating to IETF Documents \(http:\/\/trustee\.ietf\.org\/license-info\) in effect on the date of publication of this document\. Please review these documents carefully, as they describe your rights and restrictions with respect to this document\./gi,
  trust_tpl4_section6_c_iii: /This\s+document\s+may\s+contain\s+material\s+from\s+IETF\s+Documents\s+or\s+IETF\s+Contributions\s+published\s+or\s+made\s+publicly\s+available\s+before\s+November\s+10,\s+2008\..*?outside\s+the\s+IETF\s+Standards\s+Process,\s+except\s+to\s+format\s+it\s+for\s+publication\s+as\s+an\s+RFC\s+or\s+to\s+translate\s+it\s+into\s+languages\s+other\s+than\s+English/gi
}
...
export async function parse (rawText, filename) {
  ...
  try {
    ...
    // Searching for tlp4 section6_c_iii
    data.contains.pre5378Material = licenseDeclarations.trust_tpl4_section6_c_iii.test(normalizedText)
    ...
    const copyrightLicenseMatch = [...normalizedText.matchAll(licenseDeclarations.copyright_license6_b_i)]
    ...
    if (copyrightLineMatch.length) {
      ...
      data.extractedElements.copyrightDates = copyrightLineMatch.map(
