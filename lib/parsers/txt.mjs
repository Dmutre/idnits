...
export async function parse (rawText, filename) {
  ...
  try {
    ...
    // Seazg for rfc 2119 keyword declaration boilerplate
    ...
    // Searching for copyright license declaration
    data.contains.copyrightLicenseValid = licenseDeclarations.copyright_license.test(normalizedText)

    // Searching for copyright line
    const copyrightLineMatch = [...normalizedText.matchAll(licenseDeclarations.copyright_section6)]
    const copyrightLicenseMatch = [...normalizedText.matchAll(licenseDeclarations.copyright_license)]

    if (copyrightLicenseMatch.length) {
      data.possibleIssues.copyrightLicenses = copyrightLineMatch.map(m => m[0])
    }
