...
const licenseDeclarations = {
  ...
  copyrightLicense6_ii: /This document is subject to BCP 78 and the IETF Trust.s Legal Provisions Relating to IETF Documents \(https?\s*:\s*\/\/\s*trustee\s*\.\s*ietf\s*\.\s*org\s*\/\s*license\s*-\s*info\) in effect on the date of publication of this document\. Please review these documents carefully, as they describe your rights and restrictions with respect to this document\./
}
...
export async function parse (rawText, filename) {
  ...
  try {
    ...
    data.contains.copyrightLicenseValid = licenseDeclarations.revised_bsd_license6_i.test(normalizedText) || licenseDeclarations.copyrightLicense6_ii.test(normalizedText)
