...
export async function parse (rawText, filename) {
  ...
  try {
    ...
    for (const line of rawText.split('\n')) {
      ...
      const match = [...line.matchAll(licenseDeclarations.copyright_section6)]

      if (match.length > 0) {
        const copyrightText = match.map(m => m[0])

        if (copyrightText.length) data.contains.copyrightLine = true

        const copyrightYears = match.map(m => m[1] || m[2])
