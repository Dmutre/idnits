...
const convertDateToMilliseconds = (date) => {
  const pubDateObj = new Date(`${date} 01`)
  pubDateObj.setHours(0, 0, 0, 0)

  return pubDateObj.getTime()
}
...
export async function validateLineExtraSpacing (doc, { mode = MODES.NORMAL } = {}) {
  ...
  if (doc.data.possibleIssues.linesWithSpaces.length > 50) {
    if (mode === MODES.NORMAL) {
...
export async function validatePKorBM (doc, { mode = MODES.NORMAL } = {}) {
  ...
  if (doc.data.extractedElements.isPKorBM) {
    result.push(new ValidationComment(
...
export async function validateCopyrightSection (doc, { mode = MODES.NORMAL } = {}) {
  ...
  if (!doc.data.possibleIssues.copyrightLines.length) {
    result.push(new ValidationError(
  ...
  if (doc.data.possibleIssues.copyrightLines.length > 1) {
    result.push(new ValidationWarning(
 ...
 * @returns {Array} List of errors/warnings/comments or empty if fully valid
 */
export async function validateCopyrightLicense (doc, { mode = MODES.NORMAL } = {}) {
  const result = []

  if (!doc.data.possibleIssues.copyrightLicenses.length) {
    result.push(new ValidationError(
      'COPYRIGHT_LICENSE_NOT_VALID',
      'TLP-4 6.b.i or 6.b.ii license notice is not present.',
      {
        ref: 'https://trustee.ietf.org/wp-content/uploads/IETF-TLP-4.pdf'
      }))
  }

  if (doc.data.possibleIssues.copyrightLicenses.length > 1) {
    result.push(new ValidationWarning(
      'COPYRIGHT_LICENSES_MORE_THAN_ONE',
      'TLP-4 6.b.i or 6.b.ii more than one.',
      {
        ref: 'https://trustee.ietf.org/wp-content/uploads/IETF-TLP-4.pdf'
      }))
  }
 ...
 * @returns {Array} List of errors/warnings/comments or empty if fully valid
 */
export async function validateDocumentObsoletesOrUpdatesDates (doc, { mode = MODES.NORMAL } = {}) {
  const result = []

  const createdDate = doc.data.header.date
  const createdDateObj = new Date(`${createdDate.month} ${createdDate.day}, ${createdDate.year}`)

  createdDateObj.setHours(0, 0, 0, 0)

  const permitsPerYear = new Date('2008-11-10')
  permitsPerYear.setHours(0, 0, 0, 0)

  const updatesAndObsoletesRfc = [...doc.data.extractedElements.updatesRfc, ...doc.data.extractedElements.obsoletesRfc]

  if (updatesAndObsoletesRfc.length) {
    for (const rfcNumber of updatesAndObsoletesRfc) {
      const rfcCreatedDate = await fetchRemoteRfcInfo(rfcNumber)

      const rfcCreateDateInMilliseconds = convertDateToMilliseconds(rfcCreatedDate?.pub_date)

      if (rfcCreateDateInMilliseconds && rfcCreateDateInMilliseconds < createdDateObj.getTime() && rfcCreateDateInMilliseconds < permitsPerYear.getTime()) {
        if (doc.data.extractedElements.obsoletesRfc.includes(rfcNumber)) {
          result.push(new ValidationWarning(
            'OBSOLETED_RFC_PUB_DATE_NOT_VALID',
            `Using the creation date from RFC${rfcNumber}, obsoleted by this document, for RFC5378.`,
            {
              ref: 'https://trustee.ietf.org/wp-content/uploads/IETF-TLP-4.pdf'
            }))
        } else {
          result.push(new ValidationWarning(
            'UPDATES_RFC_PUB_DATE_NOT_VALID',
            `Using the creation date from RFC${rfcNumber}, updated by this document, for RFC5378.`,
            {
              ref: 'https://trustee.ietf.org/wp-content/uploads/IETF-TLP-4.pdf'
            }))
        }
...
/**
 * Validate trust material pre5378 tlp4 section 6c_iii
 * @param {Object} doc Document to validate
 * @param {Object} [opts] Additional options
 * @param {number} [opts.mode=0] Validation mode to use
 * @returns {Array} List of errors/warnings/comments or empty if fully valid
 */
export async function validateDocumentTrust6cIii (doc, { mode = MODES.NORMAL } = {}) {
  const result = []

  if (doc.data.contains.pre5378Material) {
    result.push(new ValidationWarning(
      'PRE_5378_MATERIAL_EXISTS',
      'The document appears to contain a disclaimer for pre-RFC5378 work but was first submitted on or after November 10, 2008. This disclaimer is typically required only for documents that revise or obsolete older RFCs and incorporate significant portions of text from those RFCs. If you can contact all authors of the original material and they are willing to grant BCP78 rights to the IETF Trust, you should remove the disclaimer. Otherwise, the disclaimer is necessary, and you can ignore this comment..',
      {
        ref: 'https://trustee.ietf.org/license-info'
      }))
  }
