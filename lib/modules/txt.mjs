...
export async function validateReferenceStyle (doc, { mode = MODES.NORMAL } = {}) {
  ...
  if (doc.data.references.type === 'informative') {
    doc.data.extractedElements.nonReferenceSectionRfc.forEach(ref => {
      result.push(new ValidationComment(
        'DOCUMENT_USE_INFORMATIVE_STYLE',
        `The document text uses an informative style, but the document contains references such as [RFC ${ref}]`,
        { ref: 'https://authors.ietf.org/en/required-content#references' }
      ))
    })
  } else if (doc.data.references.type === 'normative') {
    doc.data.extractedElements.nonReferenceSectionDraftReferences.forEach(ref => {
      if (/^\[\d+\]$/.test(ref)) {
        return
      }
      result.push(new ValidationComment(
        'DOCUMENT_USE_NORMATIVE_STYLE',
        `The document text uses an normative style, but the document contains references such as ${ref}`,
        { ref: 'https://authors.ietf.org/en/required-content#references' }
      ))
    })
  }
