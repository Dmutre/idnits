...
import { fetchRemoteDocInfoJson } from '../helpers/remote.mjs'
...
export async function validatePre5378Documents (doc, { mode = MODES.NORMAL } = {}) {
  ...
  if (invalidRfcNumbers.length && !doc.data.contains.licencse6_c_iii) {
...
export async function validateSubmissionComplianceLinePage (doc, { mode = MODES.NORMAL } = {}) {
  ...
  if (!doc.data.possibleIssues.submissionCompliancePage || doc.data.possibleIssues.submissionCompliancePage > 1) {
    result.push(new ValidationError(
      'SUBMISSION_COMPLIANCE_LINE_NOT_ON_THE_FIRST_PAGE',
      'Notice of conformance to BCP 78 and BCP 79 in accordance with IETF Trust Statements, December 28, 2009, Section 6.a or Trust Statements, September 12, 2009, Section 6.a does not appear on the first page',
      {
        ref: 'https://trustee.ietf.org/wp-content/uploads/IETF-TLP-4.pdf'
      }
    ))
...
export async function validateTableOfContentsAndDocumentPages (doc, { mode = MODES.NORMAL } = {}) {
  ...
  const PAGE_THRESHOLD_REQUIRING_TOC = 15
  ...
  if (!lacksTableOfContents && !isTooLong) return result
...
/**
 * Validate if document contains an acceptable paragraph noting that it is a draft
 *
 * @param {Object} doc Document to validate
 * @param {Object} [opts] Additional options
 * @param {number} [opts.mode=0] Validation mode to use
 * @returns {Array} List of errors/warnings or empty if fully valid
 */
export async function validateAcceptableParagraphNotingThatDraft (doc, { mode = MODES.NORMAL } = {}) {
  const result = []
...
export async function validateFormFeedOnSeparateLine (doc, { mode = MODES.NORMAL } = {}) {
  ...
  if (doc.data.possibleIssues.pageLineWithFormFeed.length) {
    result.push(...doc.data.possibleIssues.pageLineWithFormFeed.map((page) => new ValidationComment(
      'DOCUMENT_DIDN`T_SUCCESSFULLY_PASS_NROFF_POST_PROCESSING',
...
/**
 * Validate acceptable paragraph pointing list id
 *
 * @param {Object} doc Document to validate
 * @param {Object} [opts] Additional options
 * @param {number} [opts.mode=0] Validation mode to use
 * @returns {Array} List of errors/warnings or empty if fully valid
 */
export async function validateAcceptableParagraphPointingListId (doc, { mode = MODES.NORMAL } = {}) {
  const result = []

  if (!doc.data.possibleIssues.paragraphPointingToTheListOfCurrentId.length) {
    result.push(new ValidationError(
      'ACCEPTABLE_PARAGRAPH_POINTING_LIST_ID_MISSING',
      'Document does not contain acceptable paragraph pointing to the list of current id.',
      {
        ref: 'https://authors.ietf.org/required-content'
      }
    ))
...
export async function validateTitleUnexpectedIndentation (doc, { mode = MODES.NORMAL } = {}) {
  ...
  if (doc.data.possibleIssues.unexpectedIndentation.length) {
    result.push(new ValidationWarning(
      ...
      'Document has unexpected indentation.',
      {
        lines: doc.data.possibleIssues.unexpectedIndentation.map((obj) => obj),
        ref: 'https://authors.ietf.org/en/drafting-in-plaintext#checklist'
      }))
  }
  return result
...
export async function validateLicenseDeclarations (doc, { mode = MODES.NORMAL } = {}) {
  ...
  if (doc.data.extractedElements.licence6_b_ii.length === 0 && !doc.data.contains.revisedBsdLicense6_i) {
    result.push(new ValidationError(
  ...
  if (doc.data.slug.startsWith('draft-ietf-')) {
    if (doc.data.contains.licence6_c_i) {
      result.push(new ValidationWarning(
        'TLP4_LICENSE_NOTICE',
    ...
    if (doc.data.contains.licence6_c_ii) {
      result.push(new ValidationError(
        'TLP4_LICENSE_NOTICE',
  ...
  if (doc.data.extractedElements.licence6_b_i.length > 1 || doc.data.extractedElements.licence6_b_ii.length > 1) {
    result.push(new ValidationWarning(
