...
/**
 * Validate acceptable paragraph noting that draft
 *
 * @param {Object} doc Document to validate
 * @param {Object} [opts] Additional options
 * @param {number} [opts.mode=0] Validation mode to use
 ...
 * @returns {Array} List of errors/warnings or empty if fully valid
 */
export async function validateAcceptableParagraphPointingListIds (doc, { mode = MODES.NORMAL } = {}) {
  const result = []

  if (!doc.data.contains.draftParagraphPointingToTheListOfCurrentIds) {
    result.push(new ValidationError(
      'ACCEPTABLE_PARAGRAPH_POINTING_LIST_IDS_MISSING',
      'Document does not contain acceptable paragraph pointing to the list of current ids.',
      {
        ref: 'https://authors.ietf.org/required-content'
      }
    ))
