...
const DOCUMENTATION_IPV6_RE = /^2001:0?[Dd][Bb]8(:[0-9A-Fa-f]*)*$/i
const UNIQUE_LOCAL_IPV6_RE = /^[Ff][CcDd][0-9A-Fa-f]?[0-9A-Fa-f]?(:[0-9A-Fa-f]*)+$/i
const LINK_LOCAL_IPV6_RE = /^[Ff][Ee][89ABab][0-9A-Fa-f](:[0-9A-Fa-f]*)+$/i
  ...
  switch (doc.type) {
    case 'txt': {
      ...
      for (const ipv6Addr of ipv6Addresses) {
        if (!IPV6_RE.test(ipv6Addr)) {
          result.push(
            new ValidationWarning(
              ...
              `IPv6 address "${ipv6Addr}" is invalid.`,
              {
        ...
        if (
          !DOCUMENTATION_IPV6_RE.test(ipv6Addr) &&
          !UNIQUE_LOCAL_IPV6_RE.test(ipv6Addr) &&
          !LINK_LOCAL_IPV6_RE.test(ipv6Addr)
        ) {
          result.push(
            new ValidationWarning(
              'NON_STANDARD_IPV6_ADDRESS',
              `IPv6 address "${ipv6Addr}" does not match documentation or standard local ranges.`,
              {
